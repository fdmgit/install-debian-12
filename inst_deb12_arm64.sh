...
function inst_smart_nvme() {
    ...
    cd /root || exit
    wget  https://raw.githubusercontent.com/fdmgit/install-debian-12/main/smartmontools_7.4-2~bpo12+1_arm64.deb
    dpkg -i smartmontools_7.4-2~bpo12+1_armd4.deb
    rm smartmontools_7.4-2~bpo12+1_arm64.deb
...
function inst_kernel() {
    ...
    apt install linux-image-6.12.12+bpo-arm64 -y
    # apt install linux-headers-6.12.12+bpo-arm64 -y  # development
...
function inst_gat() {
    ...
    wget https://github.com/koki-develop/gat/releases/download/v0.19.3/gat_Linux_arm64.tar.gz
...
function inst_jos() {
    ...
    cd /usr/local/bin || exit
    wget https://github.com/kamiyaa/joshuto/releases/download/v0.9.8/joshuto-v0.9.8-aarch64-unknown-linux-musl.tar.gz
    tar -vxzf joshuto-v0.9.8-arch64-unknown-linux-musl.tar.gz -C /usr/local/bin  --strip-components=1
    chown root:root joshuto
    rm joshuto-v0.9.8-arch64-unknown-linux-musl.tar.gz
}
